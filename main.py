...
CLI USAGE EXAMPLES
========================================================================================
1) Setup & Structural Relaxation:
   $ python main.py --prefix NiTe2 setup

2) Execute Computations (SR and/or FR modes):
   $ python main.py --prefix NiTe2 run --mode SR --npool 4
   $ python main.py --prefix NiTe2 run --mode FR --npool 4

3) Post-Processing & Plotting:
   - Fermi Surface visualization:  $ python main.py --prefix NiTe2 plot --mode fs
   - Plot SR results only:         $ python main.py --prefix NiTe2 plot --mode SR
   - Plot FR results only:         $ python main.py --prefix NiTe2 plot --mode FR
   - Plot overlay (SR vs FR):      $ python main.py --prefix NiTe2 plot --mode compare
   - (Optional) Custom axes limits:$ python main.py --prefix NiTe2 plot --mode compare --emin -1.0 --emax 1.0 --dos_max 25

----------------------------------------------------------------------------------------
Advanced Run Flags
----------------------------------------------------------------------------------------
To run a specific portion of the pipeline, use the '--step' flag:
   $ python main.py --prefix NiTe2 run --mode SR --step scf --npool 4

Available pipeline steps:
- 'scf':   Step 1      — Base Self-Consistent Field calculation for the ground state.
- 'dos':   Steps 2 & 3 — NSCF calculation on a dense k-grid and DOS extraction (dos.x).
- 'fs':    Steps 4 & 5 — Un-symmetrized NSCF calculation and Fermi Surface extraction (fs.x).
- 'bands': Steps 6 & 7 — Bands calculation along the high-symmetry path and post-processing (bands.x).

Note: Before starting a completely new run, ensure temporary directories are cleared:
   $ rm -rf ./tmp*
========================================================================================
'''

'''
Comandi esatti da lanciare da terminale nell'ordine logico:
1) Setup e rilassamento strutturale: 'python main.py setup'
   (NB: Per usare un prefisso diverso: 'python main.py setup --prefix NiTe2')
2) Calcoli SR e/o FR: > SR: 'python main.py run --mode SR --npool 4'
                      > FR: 'python main.py run --mode FR --npool 4'
3) Post-Processing e grafici: > Per plottare la Fermi Surface: 'python main.py plot --mode fs'
                              > Per plottare solo il risultato SR: 'python main.py plot --mode SR'
                              > Per plottare solo il risultato FR: 'python main.py plot --mode FR'
                              > Per plottare il grafico sovrapposto SR vs FR: 'python main.py plot --mode compare'
                              > (Opzionale) Modifica limiti assi:
                                    'python main.py plot --mode compare --emin -1.0 --emax 1.0 --dos_max 25'
-----------------------------------------------------------------------------------------------------------------------
Per lanciare solo una parte della pipeline: 'python main.py run --mode [SR/FR] --step [scf/dos/fs/bands] --npool 4'

Step possibili nella fase run:
- scf: Step 1 — Esegue il calcolo di Self-Consistent Field base per la struttura elettronica dello stato fondamentale
- dos: Step 2 e 3 — Esegue il calcolo NSCF su una griglia $k$ densa ed estrae la Densità degli Stati tramite dos.x
- fs: Step 4 e 5 — Esegue il calcolo NSCF non simmetrizzato (nosym) ed estrae la Superficie di Fermi tramite fs.x
- bands: Step 6 e 7 — Esegue il calcolo delle Bande lungo il percorso ad alta simmetria nel reticolo ed effettua il
         post-processing tramite bands.x
-----------------------------------------------------------------------------------------------------------------------
Prima di un nuovo run della pipeline: 'rm -rf ./tmp ./tmp_dos ./tmp_fs ./tmp_bands'
-----------------------------------------------------------------------------------------------------------------------
Per chiedere aiuto al terminale:
> 'python main.py --help'
